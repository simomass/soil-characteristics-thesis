...
\begin{figure}[ht]
  \centering
<<label=figboh3,fig=TRUE,echo=FALSE>>=
<<plotacompWETDRY>>
@
...
Inizio discorsi porosimetria

<<label = plotacompPore, fig.width=0.5, fig.height=0.5, echo=FALSE, eval = TRUE, fig=TRUE, include = FALSE>>=
...
plot(medie, col=as.factor(df.porosimetria$TIL))
...
\caption[didascalia corta]{Distribuzione dei pori  
    \newline Gli apici del triangolo rappresentano particelle maggiori
  ...
  \SI{\Sexpr{vec.paletti.poro[2]}}{\micro\metre} (micropori)}

...
sull'aggregato inumidito per studiare il fenomeno dello \emph{slaking}.
Quindi sono stati creati due modelli: uno per i campioni inumiditi
(\emph{WET}) e uno per i camioni secchi (\emph{DRY}). I modelli
mostrano che pendenza e curvatura sono comuni per entrambi i
trattamenti.
...
I paletti macro meso micro sono a \Sexpr{vec.paletti[1]},
\Sexpr{vec.paletti[2]} rispettivamente \info{ 8rino \newline
  giustificare come mai si sono scelti questi valori invece che
  180 e 50 come suggerito da Luigi (io non me lo ricordo)}
...
Questo pu\`o essere dato dal fatto che l'appezzamento coltivato a
Biologico non \`e stato concimato organicamente, quindi la sostanza

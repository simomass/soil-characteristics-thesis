...
attuale (Fig. \ref{fig:triangolo}), modificata dal triangolo tessiturale di Whitney, \`e stata
introdotta nel 1951 \citep{SoilTr}.

\begin{figure}[ht]
  \centering
  \includegraphics[width=\textwidth]{../foto/USDA_triangolo.jpeg}
  \caption{\label{fig:triangolo}}
\end{figure}
...
Essendo il risultato dei fattori di formazione del
suolo sopra elencati, la struttura non risente particolarmente delle diverse
tecniche agronomiche, mentre la struttura viene notevolmente condizionata
...
\dubbio{guardare qui per i domains, non chiarissimo
...
appezzamenti a conduzione biologica (\emph{Bionuovo}) che per\`o non
...
\begin{figure}
  ...
  \caption[Diagramma degli appezzamenti a conduzione biologica]{Appezzamenti a conduzione biologica, i plot sono
    indicati con dei codici identificativi: \emph{OO} seguito da un
    ...
    (\emph{a} alto, \emph{m} medio, \emph{b}. Il fiume Pesa si trova
    sul margine inferiore dello schema e la collina su quello
    superiore. basso)}
...
\begin{figure}
  ...
  \caption[Diagramma degli appezzamenti a conduzione convenzionale]{Appezzamenti convenzionale, la codifica segue quella
...
\begin{figure}[ht]
  ...
  apparente: metodo \emph{Core}]{Cilindro in ottone utilizzato per
    ...
    apparente col metodo \emph{Core}, altezza = \SI{12}{\centi\metre}
...
solo una pellicola di liquido avvolgesse il campione \dubbio{nella
  bozza corretta dice nota esaustiva, ma non \`e che c'\`e molto altro
  da dire...}. Per misurare la spinta idrostatica \`e stato posto sul
piatto di una bilancia (sensibilit\`a $\pm 10^{-3}$\SI{}{\gram}), un
contenitore riempito di petrolio nel quale era immersa una navicella
di rete forata, mantenuta sospesa da un supporto metallico. Dopo aver misurato
la tara, \`e stato posto il campione entro la navicella
(Fig. \ref{fig:bilancia}) e quindi \`e misurato il peso del petrolio
spostato.
...
\migliora{Visto che non abbiamo interazioni, si puo' modificare la
  tabella \ref{tab:summaryCore} col test di Tukey e cosi anche la
  tabella \ref{tab:summaryClod}}
...
tabella.campo <-
    xtable(summary.campo,
           label = 'tab:summaryCore', align  = "llllcccc",
           caption =
               "Valori medi della densit\\`a apparente (in \\SI{}{\\gram\\per\\cubic\\centi\\metre}),
                condizionati per anno, conduzione e lavorazione " )
...
\info[inline]{Dobbiamo giustificare la differenza di numerosità del
   campione; 5, 6, 15,. Propongo di far sparire quei 15 e portarli a
   5}

 \migliora{Questa roba non è piu' valida dopo la riduzione dei dati
   2015 a 5 campioni}
 ...
 valori di densit\`a apparente misurati sul campo \migliora{Questa
   roba non è piu' valida dopo la riduzione dei dati 2015 a 5
   campioni} convenzionale siano pi\`u bassi rispetto a quelli del
 biologico.
...
\emph{Core}]{Boxplot della densit\`a apparente per i vari trattamenti:
  ...
  Convenzionale e Organico; per colore si possono distinguere le varie
...
modello. L'anova mostra che nessuno dei trattamenti presi in esame \`e
...
<<Summary_modello2, echo=FALSE, eval=TRUE, results=tex>>=
...
<<label = boxplot2, echo=FALSE, eval = TRUE, fig=TRUE, include = FALSE>>=
...
\migliora{Questa roba non è piu' valida dopo tolti i campioni}
...
analizzare tutti i 108 campioni disponibili e ogni misura \`e stata
replicata tre volte \info[inline]{Quindi sono 324 numeri ? Strano:
  dim(df.petrolio) 108 9 }.\migliora[inline]{Sono \emph{Campioni},
  \emph{misure} oppure \emph{combinazioni dei fattori dell'esperimento
  }? }

In tabella \ref{tab:summaryClod} sono riportati in sintesi le
informazioni contenute nei dati. I valori della densit\`a apparente
calcolati (mediante l'equazione \ref{eq:densitapetrolio}) sono
mediamente pi\`u elevati rispetto a quelli dell'analisi col metodo
...
Dai boxplot mostrati in Fig. \ref{fig:boxplotClod} non sono visibili
...
\caption[Boxplot dei valori di densit\`a apparente: metodo \emph{Clod}
  ...
  risultato del test di Tukey-Siegel \protect \info[inline]{al livello
    di significatività ??}}
...
interazioni tra \emph{Lavorazione} e \emph{Conduzione} non sono statisticamente
significative.
...
\migliora[inline]{Modelli da spostare ai MatMet}
...
\footnote{Rottura di aggregati di suolo (>2-\SI{5}{\mm}) secchi in
  ...
  esercita una pressione distruttiva \citep{Slaking}}. \dubbio{E
  invece il suolo wet si distrugge prima ahah. Non torna niente.}
...
I paletti macro meso micro sono a \Sexpr{vec.paletti}, rispettivamente
...
particelle in seguito alla distruzione degli aggregati]{Evoluzione
  ...
  \SI{\Sexpr{vec.paletti[2]}}{\micro\metre} \info[inline]{Figura
...
\begin{figure}[ht]
  ...
  \caption[Scattering di Mie]{In figura \`e rappresentato il fenomeno dello scattering di Mie, presa da
...
\begin{figure}[ht]
  ...
  \caption[Schema di funzionamento del Mastersizer 2000]{Rappresentazione del funzionamento del Mastersizer 2000 presa da: \href{https://plus.google.com/communities/110676150876604729660/stream/ffa7f40a-1c65-4268-9121-bb88d63f0c41}{questo sito}}
...
l'equazione \ref{eq:Msizer_Luce_Attenuata} diventa, per un insieme di particelle:
...
\chapter{Grafici dei residui dei modelli utilizzati per l'analisi dei dati}
...
\caption{Grafico quantile-quantile per i residui del modello
  ...
  \textbf{Togliere quei 'uno due tre' dal codice e metterci la roba appropriata}
}
...
\migliora{La lista della bibliografia mostra dei quadretti neri sui
  riferimenti di tipo @article. Non ho trovato soluzione 8rino}

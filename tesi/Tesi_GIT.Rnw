...
\begin{abstract}
  ...
  densit\`a apparente, distribuzione dei pori e stabilità degli aggregati.
 \textbf{ii)} Comparare gli effetti di tre lavorazioni primarie:
 aratura, rippatura e frangizollatura. I campioni di suolo sono stati
 prelevati nell'azienda agricola Montepaldi, situata a San Casciano
 val di Pesa (FI). Lo studio \`e inserito nel progetto MOLTE, un
 \textbf{long term experiment} dell'Universit\`a di Firenze dove, dal
 1991, viene condotta una sperimentazione sul sistema Biologico. I
 risultati delle analisi sulla densit\`a apparente mostrano che ci
 sono differenze statisticamente significative tra i trattamenti,
 tuttavia queste non sono significative dal punto di vista
 tecnologico. I risultati delle analisi sulla distribuzione dei pori non mostrano
 differenze statisticamente significative, mentre quelle sulla
 stabilit\`a degli aggregati mostrano indicano una maggior stabilit\`a
 dei suoli Convenzionali rispetto ai Biologici. Questo potrebbe essere
 dovuto alla mancanza di fertilizzazione dei suoli Biologici.    
\end{abstract}
...
Il concetto suolo o terreno \`e piuttosto chiaro nell'immaginario
...
\item conduzione, \emph{Biologica} \footnote{Oltre a quelli riportati
    nella lista, sono presenti anche altri quattro appezzamenti a
    conduzione biologica denominati \textit{Bionuovo} che non sono
    stati considerati in questo esperimento} \emph{Old Organic, OO}
...
identificati dalle sigle \emph{OO1, OO2, OO3, OO4} per il biologico
...
lavorazioni primarie (\textit{Lavorazione}):
\begin{itemize}
\item aratura (\emph{A}),
\item rippatura (\emph{B}),
\item frangizollatura (\emph{C}).
\end{itemize}
...
\begin{figure}
  ...
  \caption{Appezzamenti convenzionale, la codifica segue quella
    descritta in Fig. \ref{fig:OO_sito} \label{fig:CO_sito}}
...
Per quanto riguarda i dati provenienti dalle analisi di distribuzione dei pori e di
stabilit\`a degli aggregati, questi sono per loro stessa natura
...
assunzioni di normalit\`a \citep{stefanini2007introduzione}. Non essendo state
riscontrate interazioni statisticamente significative tra i parametri
considerati, esse non compaiono nei modelli sotto riportati
...
La variabile categorica \emph{anno}, \`e stata utilizzata solo nel
modello \emph{Core} in quanto i per l'anno 2015 sono disponibili solo
per questo metodo.

Nel modello adattato per i dati della stabilit\`a degli aggregati,
sono state aggiunte la componente tempo (di disgregazione) e tempo al quadrato.
...
Sui metodi \emph{Core} e \emph{Clod} \`e stato applicato ai gruppi
oggetti di studio, data la natura additiva dei modelli, il test di
Siegel-Tukey utilizzando i pacchetti 'agricolae' \citep{agricolae} e
...
\caption{Boxplot della densit\`a apparente per il metodo core: 
  ...
  per colore si possono distinguere le varie lavorazioni (arato, frangizollato e rippato); la riga rossa rappresenta la met\`a del valore della densit\`a reale
...
tabella.petrolio <-
    xtable(summary.petrolio,
           label = 'tab:summary petrolio', align  = "lllcccc",
           caption = 

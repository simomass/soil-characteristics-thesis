...
\dubbio{Simo \newline per ora non so cosa scriverci, mi verrà in mente qualcosa}
...
\subsection{Densità}
Sia per il metodo \emph{Core} che per quello \emph{Clod} NON sono state
...
\caption[Boxplot dei valori di densit\`a apparente: metodo \emph{Clod}
  ...
  risultato del test di Tukey-Siegel ($p<0.05$)}
...
\footnote{Rottura di aggregati di suolo (>2-\SI{5}{\mm}) secchi in
  aggregati pi\`u piccoli (<\SI{0.25}{\mm}) quando sono
  improvvisamente immersi in acqua. Lo slaking avviene quando gli
  aggregati non sono forti abbastanza da resistere agli stress interni
  causati dal rapido ingresso dell'acqua che, comprimendo l'aria,
  esercita una pressione distruttiva \citep{Slaking}}.
...
particelle in seguito alla distruzione degli aggregati]{Evoluzione
  ...
  \SI{\Sexpr{vec.paletti[2]}}{\micro\metre} \info[inline]{8rino Figura
    troppo grossa: sistemala come la Fig. \ref{fig:boxplotCore}} }
\end{figure}
...
require(lattice)
hist.esteso <- histogram(~Volume.NA|SUBJECT, data=df.rawdata)

# xyplot(Volume.NA ~ log(Radius.NA, base=10) |MAN+TIL, data=df.rawdata)
hist.media <- histogram( ~ Volume.NA |MAN+TIL, data=df.rawdata)
...
\info{Simo Mi faccio un riassuntino dell'introduzione del libro intanto}

...
\info{Simo: \newline Sentire Luigi per scrivere materiali e metodi
  + Eventuale appendice per il funzionamento della porosimetria a mercurio}
\subsection{Protocollo di misura}
...
I valori della densit\`a apparente
calcolati mediante l'equazione \ref{eq:densitapetrolio} sono
mediamente pi\`u elevati (media;
\Sexpr{round(mean(df.petrolio$densita.apparente, na.rm=T),2)}) 
rispetto a quelli dell'analisi col metodo
\emph{Core} riportati a pag. \pageref{tab:summaryCore}.
...
\migliora{8rino da rivedere \newline Come nel metodo \emph{Core},
  anche in questo caso il numero di campioni \`e stato sufficiente a
  determinare la differenza significativa di due trattamenti
  nonostante si differenzino per la seconda cifra decimale.}
...
PESI <- read.table("~/Simo_GIT/dati_elaborati/pesoaggre.csv", sep = ";")$x

df.data <- data.frame(
    df.data[,1:6],
    MICROPORI = (df.data[,7]-df.data[,8])/PESI, 
    MESOPORI = (df.data[,8]-df.data[,9])/PESI, 
    MACROPORI = df.data[,9]/PESI
#    TOT = df.data[,7]/PESI
)

## lm.micropori <- 
##     with(df.data, lm(MICROPORI ~ MAN)) #+ TIL))
## anova(lm.micropori)
## summary(lm.micropori)
## lm.macropori <- 
##     with(df.data, lm(MACROPORI ~ MAN)) #+ TIL))
## anova(lm.macropori)
...
## lm.ratio <-
##     lm(MACROPORI/MICROPORI ~ MAN*TIL, data = df.data)
## anova(lm.ratio)
...
tutti in valori centrali, quindi, i pori sono distribuiti equamente tra
le tre classi dimensionali (particelle maggiori
  di \Sexpr{vec.paletti.poro[1]} (macropori), comprese tra
  \Sexpr{vec.paletti.poro[1]} e \Sexpr{vec.paletti.poro[2]} (mesopori) e inferiori a
  \Sexpr{vec.paletti.poro[2]} \SI{}{\micro\metre} (micropori)).
...
\begin{figure}[ht]
   \centering
<<label = figurina, fig=TRUE, echo=FALSE, width=5.4, height=5.4>>=
<<plotacompPore>>

@

\caption[didascalia corta]{Diagramma ternario della distribuzione dei pori. Gli apici del triangolo rappresentano le classi dimensionali in cui le particelle sono divise in: inferiori
  di \Sexpr{vec.paletti.poro[1]} (macropori), comprese tra
  \Sexpr{vec.paletti.poro[1]} e \Sexpr{vec.paletti.poro[2]} (mesopori) e superiori a
  \Sexpr{vec.paletti.poro[2]} \SI{}{\micro\metre} (micropori)}
...
lm.totale <- 
    lm(MICROPORI+MACROPORI+MESOPORI ~ MAN, data = df.data)
anova.totale <- 
    anova(lm.totale)
...
permette di affermare il numero di campioni non \`e sufficiente per
...
\chapter{Discussione}
\section{Densit\`a apparente}
\info{Devo trovare la voglia di leggere un po' di bibliografia, anche
  se mi pare che non ci sia niente di ch\'e.}

Il metodo \emph{Clod} metodo \`e noto fornire dei valori di densit\`a
apparente pi\`u elevati rispetto a quelli misurati utilizzando il
metodo \emph{Core}, questo \`e dovuto al fatto che nella misura non
sono considerate le fratture \citep{tisdall1951comparison}.

...
coefs <-  ilrInv(coef(modelloMsizer), orig=Y.2)
##############################################################
CO <- round(matrix(coefs[1,] +
     acomp(coefs[3,])*on.ultra.off +
     acomp(coefs[4,])*on.ultra.off^2,
...
qqnorm(ilrInv(resid(modelloMsizer), orig=Y))
...
coefs <-  ilrInv(coef(modelloPoro), orig=Y)
alpha <- 0.05
@

\info{Simo Ho messo intanto tutto l'import, poi per i grafici aspetto il prof}

Inizio porosimetria

<<label = plotacompPore, fig.width=0.5, fig.height=0.5, echo=FALSE, eval = TRUE, fig=TRUE, include = FALSE>>=
 plot(Y, col=as.factor(df.analisi$TIL))
 intercetta <-
  ilrInv(coef(modello)[1,], orig=Y)
 bi <-
     ilrInv(rbind(0, coef(modello)[-1,]), orig=Y)
@
 
\begin{figure}[ht]
   \centering
<<label = figurina,fig=TRUE,echo=FALSE>>=
<<plotacompPore>>
@
\end{figure}
...
sommario <- 
    summary(lm(apply(df.data[,7:9], 1, sum) ~ df.data$TIL))
tabella_sommario <- 
    xtable(sommario,
           caption = "T-table modello porosit\\`a", label = 'tab:summary pori', align = "lcccc")
...
\caption{Grafico quantile-quantile per i residui del modello
  composizionale per la stabilità degli aggregati \textbf{Togliere
    quei 'uno due tre' dal codice e metterci la roba appropriata} }

...
struttura del terreno come \textit{il particolare modo di aggregarsi dei
...
\begin{table}[ht]
  \caption[corta]{Classificazione dei pori del terreno secondo le dimensioni
...
L'esperimento oggetto di questa tesi fa parte del progetto MoLTE (Montepaldi Long Term
...
pianeggianti) con caratteristiche tessiturali li caratterizzano tra i \textit{medio impasto
  limo-argillosi} e gli \textit{argillosi} con presenza
variabile di scheletro di piccole e medie dimensioni.


Il sito sperimentale \`e costituito da due agro ecosistemi, da qui in
avanti definiti come \textit{Conduzione}:
\begin{itemize}
...
\begin{figure}
  ...
  \caption[corta]{Appezzamenti biologico, i plot sono indicati con dei codici
...
\begin{figure}
  ...
  \caption[corta]{Appezzamenti convenzionale, la codifica segue quella
...
\begin{figure}[ht]
  ...
  \caption[corta]{Cilindro in ottone utilizzato per prelevare i campioni da
...
\begin{figure}[ht]
  ...
  \caption[corta]{Schema dello strumento Mastersizer 2000,
...
\begin{figure}[ht]
  ...
  \caption[corta]{In figura alcuni esempi di curve di distribuzione
...
\caption[corta]{Boxplot della densit\`a apparente per i vari trattamenti: a
  sinistra sono presenti le misurazioni dell'anno 2015, a destra
  ...
  lavorazioni (arato, frangizollato e rippato).La riga rossa indica la
  media generale }
...
<<boxplot2>>
@
\caption[corta]{Boxplot densit\`a apparente in cui \`e stato eliminato
...
<<boxplotpetr>>
@
\caption[corta]{Boxplot dei valori di densit\`a apparente ottenuti col metodo
...
\caption[corta]{Evoluzione della distribuzione granulometrica delle
  particelle liberatisi in seguito a: \newline i) inizio misura
  (immersione, punti indicati dalle lettere minuscole \emph{c} e
  \emph{o}) \newline ii) accensione sonicatore (lettere \emph{W} e
  \emph{D}) \newline iii) fine ciclo. \newline Gli apici del triangolo
  rappresentano particelle maggiori di \Sexpr{vec.paletti[1]} (MACRO),
  comprese tra \Sexpr{vec.paletti[1]} e \Sexpr{vec.paletti[2]} (MESO)
  e inferiori a \SI{\Sexpr{vec.paletti[2]}}{\micro\metre} 
  \info[inline]{Figura troppo grossa: sistemala come la
    Fig. \ref{fig:boxplotCore}}
  }
...
\begin{figure}[ht]
  ...
  \caption[corta]{In figura \`e rappresentato il fenomeno dello scattering di Mie, presa da
...
\begin{figure}[ht]
  ...
  \caption[corta]{Rappresentazione del funzionamento del Mastersizer 2000 presa da:
...
<<qqplotAcomp>>
@
  \caption[corta]{qqplot del modello composizionale densimetria dinamica}

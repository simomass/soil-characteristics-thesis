...
\documentclass[11pt, a4paper, openright, titlepage, final]{book}
...
\begin{frontespizio}
  ...
  \Titolo{25 anni di conduzione Biologica: uno studio di fisica del suolo}
  \NCandidato{Candidato}
  \Candidato{Simone Massenzio}
  \NRelatore{Relatore}{}
  \Relatore{Dott. Ottorino-Luca Pantani}
...
\begin{abstract}
  ...
  pi\`u di venti anni) studiando tre propriet\`a fisiche del suolo:
  densit\`a apparente, distribuzione dei pori e stabilità degli
  aggregati.  \textbf{ii)} Comparare gli effetti di tre lavorazioni
  primarie: aratura, rippatura e frangizollatura. I campioni di suolo
  sono stati prelevati nell'azienda agricola Montepaldi, situata a San
  Casciano val di Pesa (FI). Lo studio \`e inserito nel progetto
  MOLTE, un \textbf{long term experiment} dell'Universit\`a di Firenze
  dove, dal 1991, viene condotta una sperimentazione sul sistema
  Biologico. I risultati delle analisi sulla densit\`a apparente
  mostrano che ci sono differenze statisticamente significative tra i
  trattamenti, tuttavia queste non sono significative dal punto di
  vista tecnologico. I risultati delle analisi sulla distribuzione dei
  pori non mostrano differenze statisticamente significative, mentre
  quelle sulla stabilit\`a degli aggregati mostrano indicano una
  maggior stabilit\`a dei suoli Convenzionali rispetto ai
  Biologici. Questo potrebbe essere dovuto alla mancanza di
  fertilizzazione dei suoli Biologici.
\end{abstract}
...
\begin{otherlanguage*}{english}
  \begin{engabstract}
    ...
    carried out an experimentation on the organic farming systems.  The
    results of the analysis of the bulk density, shows that there are
    statistically significant differences between two farming systems,
    however these are not technologically significant. The results of
    the pore distribution are not statistically significant. The
    aggregate stability shows that the conventional's soil is more
    stable than the organic one. This could be due to the lack of
    fertilisation in the organic soil.
  \end{engabstract}%
\end{otherlanguage*}
...
Nel terreno è importante la distribuzione granulometrica, o tessitura,
in cui le particelle di diametro superiore a \SI{2}{\milli\metre} sono
dette scheletro, quelle tra \SI{2}{\mm} e \SI{50}{\micro\metre} sono
dette sabbia, quelle tra \SI{50}{\micro\metre} e \SI{2}{\micro\metre}
sono dette limo e quelle inferiori a \SI{2}{\micro\metre} sono dette
argilla. La tessitura \`e usualmente rappresentata mediante il
triangolo tessiturale introdotta da Whitney nel 1911 e adottata
...
elencati, la struttura non risente particolarmente delle diverse
...
La densit\`a apparente \`e un importante parametro del suolo collegato
con la tessitura e la struttura. Mentre il valore della densit\`a
reale è, ai fini pratici, costante e considera solo la frazione
minerale\footnote{La densit\`a delle particelle solide non \`e
  uniforme per tutti i componenti (varia da circa 2 a oltre 3) ma, in
  considerazione del fatto che nel suolo sono presenti prevalentemente
  i silicati (con densit\`a che varia da 2,5 a 2,8), si assume il
  valore di \SI{2.65}{\gram\per\cubic\centi\metre}
  \citep{radaelli2001chimica}}, la densit\`a apparente considera anche
gli spazi vuoti, ovvero i pori.
...
La porosit\`a del terreno è un ulteriore parametro fondamentale: il
volume dei pori rispetto il volume della fase solida influenza
notevolmente il contenuto idrico e la lavorabilit\`a del terreno, la
sua misura può aiutare la quantificazione dell’impatto delle pratiche
colturali sul suolo ed è il miglior indicatore per la qualit\`a della
struttura \citep{pagliai2002soil}. La quantificazione dei pori in
termini di forma, spazio, dimensione, continuit\`a, orientamento e
arrangiamento permette di definire la complessit\`a della struttura e
fa capire le modificazioni indotte dalle pratiche colturali; in questo
modo è possibile indicare quali di queste siano più compatibili con la
protezione ambientale. La porosit\`a \`e correlata con la resistenza
...
dei pori e la distribuzione della grandezza sono inoltre correlate a
propriet\`a chimiche, biochimiche e biologiche (come l'attivit\`a
enzimatica e la crescita delle radici).
...
quanto riguarda le forze che tengono insieme i \emph{domains}.
...
tessitura fine. Quando questa categoria di pori raggiunge o supera il
70-80\% della porosit\`a totale, è indice di cattiva struttura e non
favorisce la crescita delle piante. Questo è dovuto al fatto che le
crepe superficiali che si vengono a creare a seguito di una pioggia,
quando la stabilit\`a degli aggregati non è buona, appartengono a
questa classe. Se i pori grandi, medi e piccoli sono in proporzione
ottimale, si osserva un concomitante immagazzinamento/conduzione di
acqua e aria che permette un'adeguata crescita radicale. Questa
proporzione non è stata ancora definita a causa del fatto che alle
radici servono comunque i pori di
...
tecnologica, oggi divenuta convenzionale. L'agricoltura convenzionale
esaspera il concetto di massimizzazione del rendimento economico, \`e
tecnologicamente orientata, utilizza ampie superfici spesso coltivate
a monocoltura (con conseguente perdita della biodiversit\`a) e
richiede ingenti quantit\`a di concimi e fertilizzanti, normalmente di
...
popolazione di met\`a '900 \citep{Ciuffoletti_2014}; ma, al tempo
stesso, ha portato a problemi di tipo ambientale e sanitario:
patologie dovute all'esposizione ad antiparassitari, riduzione della
biodiversit\`a degli ecosistemi, inquinamento di aria, suolo e acqua,
...
Scopo di questa tesi è la verifica di due ipotesi:
\begin{itemize}
...
Casciano Val di Pesa) e interessa una superficie leggermente declive
di circa \SI{15}{\hectare} a \SI{90}{\meter} s.l.m. .
\begin{figure}[ht]
  \centering
  \includegraphics[width=\textwidth]{../foto/Panoramica_lavorazioni.jpeg}
...
pianeggianti) con caratteristiche tessiturali li caratterizzano tra i
...
inerbita separa il sistema \emph{Bionuovo} da quello
\emph{Convenzionale}.
...
Ogni lavorazione \`e stata replicata tre volte (\emph{Plot}), quindi
...
\begin{figure}
  ...
  biologica]{Appezzamenti a conduzione biologica, i plot sono indicati
    ...
    medio, \emph{b}. Il fiume Pesa si trova sul margine inferiore
    dello schema e la collina su quello superiore. basso)}
...
volumi) che con il metodo \emph{Clod} (utilizzando piccoli aggregati).
...
Con i dati rilevati, \`e stato ricavato il valore della densit\`a
...
\begin{tabular}{rp{12cm}}
  ...
  $P_{secco}$=       & peso del terreno vagliato ed essiccato (\SI{}{\kilogram}).
\end{tabular}
\vspace*{3em}
...
effettuata su aggregati di piccole dimensioni(3-\SI{4}{\cm} di
diametro).  Il metodo \emph{Clod} viene anche detto metodo per spinta
idrostatica \citep{Monnier_1973}. La procedura \`e consiste nel
mettere per una notte a bagno nel petrolio degli aggregati di terreno
...
elevati rispetto a quelli misurati utilizzando il metodo Core, questo \`e
...
2000 che li disgrega meccanicamente.
...
dell'energia cinetica al campione da parte della pompa, \`e
fondamentale per la determinazione della resistenza alla disgregazione
degli aggregati. Infatti effettuando una lettura a intervalli regolari
(quindi analizzando la variazione della distribuzione nel tempo) \`e
possibile determinare la capacit\`a degli aggregati di resistere a
...
acquisire $I_0$ (Eq. \ref{eq:BeerLambert}).
...
Per studiare il fenomeno dello \emph{slaking} \footnote{Rottura di
...
due volte: una sul campione talquale e una sul campione
...
\item 12 distribuzioni dal tempo 1 al tempo 12 per il campione umido
  senza sonicatura;
\item 12 distribuzioni dal minuto 13 al minuto 24 per il campione
  umido con la sonicatura attiva.
\end{itemize}
...
\begin{figure}[ht]
  ...
  dinamica]{In figura alcuni esempi di curve di distribuzione
    granulometrica dinamica: sull'asse delle ascisse sono presenti i
    valori dei diametri delle varie particelle misurate dallo
    strumento in \SI{}{\micro\metre}, scala logaritmica; sull'asse
    delle ordinate sono presenti i relativi valori di volume in
...
ad un elaboratore DOS per la raccolta e la elaborazione dei dati.
...
\subsection{Densit\`a}
Sia per il metodo \emph{Core} che per quello \emph{Clod} \textbf{non} sono state
...
Una volta ottenuto il modello definitivo \`e stata effettuata una
...
\subsection{Dinamica e porosimetria}
...
Questi sono stati raggruppati in tre range dimensionali
(macroaggregati, mesoaggregati, microaggregati e macropori, mesopori,
micropori). Le dimensioni sono state definite in base alla
letteratura (\citet{paul2014soil}, \citet{greenland1977soil}).
...
la densit\`a apparente mediante la formula \ref{eq:DensitaApparente}.
...
condizionati per \\emph{Anno}, \\emph{Conduzione} e \\emph{Lavorazione} " )
...
\begin{figure}[hb]
  ...
  \emph{Core}]{Boxplot della densit\`a apparente per i vari trattamenti:
    a sinistra sono presenti le misurazioni dell'anno 2015, a destra
    quelle del 2016; all'interno dei due anni sono suddivisi in
    \emph{Convenzionale} e \emph{Organico}; per colore si possono distinguere le varie
...
modello e indica la non significatività dei trattamenti presi in esame.
...
In tabella \ref{tab:summaryClod} sono riportate in sintesi le
informazioni contenute nei dati. Gli stsssi dati sono mostrati
graficamente in Fig. \ref{fig:boxplotClod}.
...
rispetto a quelli dell'analisi col metodo \emph{Core} riportati a
...
@
  ...
  ]{Boxplot dei valori di densit\`a apparente ottenuti col metodo di
    analisi \emph{Clod}; gli appezzamenti condotti convenzionalmente (a
    sinistra) hanno valori leggermente più alti degli appezzamenti
...
caption = ' Tabella ANOVA per il metodo \\emph{Clod}'
)
...
Nel modello \`e presente l'elemento tempo al quadrato il quale, come
...
Di conseguenza sono stati creati due modelli: uno per i campioni
inumiditi (\emph{WET}) e uno per i campioni secchi (\emph{DRY}). Dai
modelli \`e possibile vedere che pendenza e curvatura sono comuni
per entrambi i trattamenti.

I secchi generano, come atteso, particelle più grandi in seguito allo
slacking. 

Sia dalle analisi su gli aggregati secchi che da quelle su gli
aggregati umidi, risulta che gli aggregati provenienti da appezzamenti
condotti convenzionalmente, non appena vengono lasciati cadere in
acqua, si disgregano generando particelle più grosse di quanto non
facciano gli aggregati "biologici".
...
caption = 'Evoluzione della distribuzione granulometrica
delle particelle liberatisi dagli aggregati
precedentemente inumiditi in seguito a:
...
label = 'tab:anova_compWET', align = 'rrrrrrr',
caption = 'Aggregati per i dati DRY '
)
...
I dati raccolti sono stati analizzati mediante tecniche di analisi
...
Dai dati \`e possibile osservare come tra i trattamenti non ci siano
...
@
  \caption[didascalia corta]{Diagramma ternario della distribuzione dei
    ...
    in cui le particelle sono divise in: inferiori di
...
La tabella ANOVA \ref{tab:poros_anova}, conferma l'analisi visuale:
...
tabella_sommario <- 
xtable(sommario,
caption = "t-table modello porosit\\`a", label = 'tab:Poro_medie', align = "llcccc")
...
label = 'tab:poros_anova', align = 'rrrrrrr',
...
\begin{appendices}
  \chapter{Calcolo della concentrazione delle componenti
    granulometriche con Mastersizer 2000}
  ...
  \chapter{Dati composizionali: cosa sono e come vengono analizzati}
...
\chapter{Grafici dei residui dei modelli utilizzati per l'analisi dei
    dati}

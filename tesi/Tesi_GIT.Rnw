...
@
\caption{Grafico quantile-quantile per i residui del modello
  composizionale per i datti di porosimetria}
  \label{fig:qqplot_porosimetria}
\end{figure}

\end{appendices}

\migliora{8rino La lista della bibliografia mostra dei quadretti neri sui
  riferimenti di tipo @article. Non ho trovato soluzione}
